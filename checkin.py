...
def load_config() -> Tuple[str, List[str], str]:
    ...
    if not raw_cookies_env:
        ...
        cookies_list = [
            cookie.strip() for cookie in raw_cookies_env.split("&") if cookie.strip()
        ]
        if not cookies_list:
            raise ValueError(
                f"环境变量 '{ENV_COOKIES}' 已设置，但未包含任何有效的 Cookie。"
            )

    if not exchange_plan_env:
        logger.warning(
            f"环境变量 '{ENV_EXCHANGE_PLAN}' 未设置，将使用默认兑换计划 'plan500'。"
        )
        ...
        if exchange_plan_env in EXCHANGE_POINTS:
            ...
            logger.warning(
                f"环境变量 '{ENV_EXCHANGE_PLAN}' 的值 '{exchange_plan_env}' 无效，将使用默认兑换计划 'plan500'。"
            )
            exchange_plan = "plan500"
    ...
    try:
        if method.upper() == "POST":
            response = requests.post(
                url, headers=session_headers, data=json.dumps(data)
            )
        ...
        if not response.ok:
            logger.warning(
                f"向 {url} 发起的请求失败，状态码 {response.status_code}。响应内容: {response.text}"
            )
            return None
        return response
...
def checkin_and_process(
    cookie: str, exchange_plan: str
) -> Tuple[str, str, str, str, str, bool]:
    """执行签到并处理后续操作。

    Returns:
        (status_msg, points_gained, remaining_days, remaining_points, exchange_msg, checkin_ok)
        checkin_ok 为 True 表示签到成功或重复签到，False 表示签到失败。
    """
    ...
    checkin_response = make_request(
        CHECKIN_URL, "POST", HEADERS_TEMPLATE, CHECKIN_DATA, cookies=cookie
    )
    ...
    if status_response:
        try:
            ...
            logger.error(
                f"解析剩余天数时出错: {status_data.get('data', {}).get('leftDays', 'unknown')}"
            )
    ...
    if current_points_numeric >= required_points:
        ...
        logger.info(
            f"积分不足以兑换 {exchange_plan}。所需: {required_points}, 当前: {current_points_numeric}"
        )
...
def format_push_content(results: List[Dict[str, str]]) -> Tuple[str, str]:
    ...
    fail_count = sum(
        1 for r in results if "失败" in r["status"] or "失败" in r["exchange"]
    )
    ...
    try:
        ...
        if not cookies_list:
            ...
            for idx, cookie in enumerate(cookies_list, 1):
                ...
                status, points, days, points_total, exchange, checkin_ok = (
                    checkin_and_process(cookie, exchange_plan)
                )
                if not checkin_ok:
                    has_failure = True
                results.append(
